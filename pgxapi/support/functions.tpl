...
func doScan{{.Type.U}}List(req require.Requirement, rows pgxapi.SqlRows, qLog func(error) error) ([]{{.Type}}, error) {
	var v {{.Type}}
	list := make([]{{.Type}}, 0, 10)
...
// The result set should have been produced via a SELECT statement on just one column.
func doScan{{.Type.U}}PtrList(req require.Requirement, rows pgxapi.SqlRows, qLog func(error) error) ([]{{.Type}}, error) {
	var v sql.Null{{.NT}}
	list := make([]{{.Type}}, 0, 10)
